...
namespace IDL4_EA_Extension
{
    public class IDLGenAction : UserActionInterface
    {
        ...
        public void OnIgnoreAggregationNoneOption(bool checkedBox)
        {
            Main.setIgnoreAggregationNoneOption(checkedBox);
        }
    ...
    public class Main
    {
        ...
        private static bool idlInsertPlaceholderForUnresolvedType = IDLVersions.defaultInsertPlaceholderForUnderTypeValue;
        private static bool ignoreAggregationNoneOption      = IDLVersions.defaultIgnoreAggregationNoneOptionValue;
            ...
            HashSet<string> uncheckedElem, String fullPath)
        {
            ...
            else if (classElem != null)
            {
                ...
                {
                    Main.GenIDL_Class(repository, classElem, output, elementNames.Length - 1, uncheckedElem, pathToElement, null);
                }
            }
            for (int i = 1; i < elementNames.Length - 1; ++i)
            {
            ...
            foreach (Element e in package.Elements)
            {
                if (!IsElementEnum(e))
                {
                        ...
                        && GenIDL_MustGenerateClass(repository, e, packageFullName, uncheckedElem, null,  output, depth) )
                    {
                        ...
                        GenIDL_ReportUngeneratedInnerClasses(repository, package, e, output, depth, completionDB);
                    }
                }
            }
        ...
        private static void GenIDL_ReportUngeneratedInnerClasses(Repository repository, Package package, Element classElem,
            TextOutputInterface output, int depth, CodegenCompleteDatabase completionDB)
        {
            foreach (Element child in classElem.Elements)
            {
                if (!IsElementEnum(child))
                {
                    if (completionDB.IsClassGenerated(child) == false)
                    {
                        GenIDL_DependenciesAlreadyGenerated(repository, child, output, completionDB, true);
                        GenIDL_ReportUngeneratedInnerClasses(repository, package, child, output, depth, completionDB);
                    }
                }
            }
        }
        ...
        {
            return classElem.HasStereotype("XSDunion") || HasTagWithValue(classElem, "modelGroup", "choice");             
        }
        ...
        */
            ...
            HashSet<String> uncheckedElem, Dictionary<long, bool> relevantModules, CodegenCompleteDatabase completionDB)
        {
            ...
            // Generate enums and typedef for structures
            if (idlMappingDetail >= IDLMappingDetail.IDL_DETAILS_FULL)
            {
            ...
            HashSet<String> uncheckedElem, Dictionary<long, bool> relevantModules, CodegenCompleteDatabase completionDB)
        {
            ...
            foreach (Element e in package.Elements)
            {
                if (GenIDL_MustGenerateClass(repository, e, packageFullName, uncheckedElem, completionDB, output, depth))
                {
                    ...
                    {
                        GenIDL_Class(repository, e, output, depth + 1, uncheckedElem, packageFullName, null);
                        ++generatedItemCount;
                        completionDB.AddGeneratedClass(e);
                    }
                    else
                    {
                        ...
                        notGeneratedClassCount += GenIDL_NestedComplexElementTypedefs(repository, e, null, uncheckedElem, output, depth, completionDB);
                    }
                }
            }
        ...
        /* 
         * This handles regular structured (complex) types that appear defined in-line inside ther complex types
         * something that is allowed in XSD.
         */
        private static int GenIDL_NestedComplexElementTypedefs(Repository repository, Element classElem,
            ...
            CodegenCompleteDatabase completionDB)
        {
            ...
            {
                containerName = IDL_NormalizeUserDefinedClassifierName(containerName)
                                + "_" + IDL_NormalizeUserDefinedClassifierName(classElem.Name);
            }
            else
            {
                containerName = IDL_NormalizeUserDefinedClassifierName(classElem.Name);
            }

            if (idlMappingDetail >= IDLMappingDetail.IDL_DETAILS_FULL) { 
                output.OutputTextLine(depth + 1, "// Evaluating element \"" + containerName + "\"");
            }

            foreach (Element child in classElem.Elements)
            {
                if (GenIDL_MustGenerateClass(repository, child, containerName, uncheckedElem, completionDB, output, depth) )
                {
                    if (GenIDL_DependenciesAlreadyGenerated(repository, child, output, completionDB, false))
                    {
                        GenIDL_Class(repository, child, output, depth + 1, uncheckedElem, null, containerName);
                        completionDB.AddGeneratedClass(child);
                    }
                    else
                    {
                        GenIDL_NestedComplexElementTypedefs(repository, child, containerName,
                            uncheckedElem, output, depth, completionDB);
                        ++notGeneratedClassCount;
                    }
                }
            }
            ...
            TextOutputInterface output, int depth)
        {
            ...
            foreach (EA.Attribute child in classElem.Attributes)
            {
                ...
                if (typeName.Equals(""))
                {
                    if (idlMappingDetail >= IDLMappingDetail.IDL_DETAILS_FULL)
                    {
                        output.OutputTextLine(depth, "// Mapping to \"octet\" because attribute does not have explicit type");
                    }
                    typeName = "octet";
                }
            ...
            TextOutputInterface output, int depth)
        {
            ...
            if ( Main.ignoreAggregationNoneOption && (sourceElemEnd.Aggregation == 0) )
            {
                if (explain)
                {
            ...
            */
            ...
            if ((referencedElemEnd.Containment.Equals("Reference"))
                 || (referencedElemEnd.Containment.Equals("Unspecified") && ( sourceElemEnd.Aggregation != 2) ))
            {
            ...
            Element sourceClass, EA.Connector conn, TextOutputInterface output, int depth)
        {
            ...
            else if (cardinality.Equals("") || cardinality.Equals("1") || cardinality.Equals("0") || cardinality.Equals("1..1"))
            {
                // Map to regular member. No sequence, no optional
                if (idlMappingDetail >= IDLMappingDetail.IDL_DETAILS_FULL)
                {
                    output.OutputTextLine(depth, "/* Mapping to regular member because relation cardinality is \"\", \"1\", \"0\", or \"1..1\" */");
                }
            }
            else
            {
            ...
            if (includeAsReference)
            {
                ...
                if (idlMappingDetail >= IDLMappingDetail.IDL_DETAILS_FULL)
                {
                    output.OutputTextLine(depth, "/* Mapping to Shared because Containment is 'Reference' or Aggregation is 'Shared' or 'None' */");
                }
            }
        ...
        {           
            ...
            if (baseClassName == null)
            {
                if (Main.idlInsertPlaceholderForUnresolvedType)
                { 
        ...
        /**
         * Checks if a specific Tag name is there and has the specified value
         */
        private static bool HasTagWithValue(EA.Element elem, String tagName, String tagValue)
        {
            foreach ( TaggedValue tag in elem.TaggedValues )
            {
        ...
        private static void GenIDL_Class(Repository repository, Element classElem,
            TextOutputInterface output, int depth,
            HashSet<String> uncheckedElem, String elementPath, String containerName)
        {
            if ( IsXSDUnion(classElem) )
            {
                GenIDL_Union(repository, classElem, output, depth, uncheckedElem, containerName);
            }
            else
            {
                GenIDL_Struct(repository, classElem, output, depth, uncheckedElem, containerName);
            }
        }

        private static void GenIDL_Union(Repository repository, Element classElem,
                                          TextOutputInterface output, int depth,
                                          HashSet<String> uncheckedElem, String containerName)
        {
            String className = IDL_NormalizeUserDefinedClassifierName(classElem.Name);
            if ( containerName != null )
            {
                className = IDL_NormalizeUserDefinedClassifierName(containerName) + "_" + className;
            }
        ...
        {
            Main.idlInsertPlaceholderForUnresolvedType = b;
        }

        internal static void setIgnoreAggregationNoneOption(bool b)
        {
            Main.ignoreAggregationNoneOption = b;
        }
    }
}
